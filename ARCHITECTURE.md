...
if ( ! is_preview() && is_built_with_elementor() ) {
    wp_enqueue_script('ml4e-frontend-js', ...);
}
```
...
wp_enqueue_script('ml4e-editor-js', ...);
```
...
React component mounts 
  ...
  → Nonce from window.ML4E.storeApiNonce 
  → Products setState() 
...
- Batch apply mechanism: temporarily disables `renderOnChange` and `change:ml4e_layout` listener, calls `settingsModel.set()` atomically, then `renderUI()` for CSS + `updateInstance()` for React
...
### `window.ML4E` (Localized PHP Data)
...
|---------|-----------|-----------|----------|
| Focal Point | `Focal_Point` | `ml4e_focal_point` | Image focal-point picker |
| Product Select | `Product_Select` | `ml4e_product_select` | Single-product selector with search |
| Element Sorting | `Element_Sorting` | `ml4e_sorter_label` | Drag-to-reorder element visibility list |
| Saved Setups | `Saved_Setups` | `ml4e_saved_setups` | Save/load/delete layout+style presets |
...
**Persistence:**
- Stored in `wp_options` as JSON string under key `ml4e_content_layout_setups`
...
- Temporarily disables `renderOnChange` and `change:ml4e_layout` listener
...
- Output: `{ '--ml4e-title-size-desktop': '24px', '--ml4e-title-size-tablet': '20px', '--ml4e-title-size-mobile': '18px' }`
...
- Nonce passed to React via `window.ML4E.storeApiNonce`
...
window.ML4E.placeholderImg = 'plugins/.../woocommerce-placeholder-300x300.png';

// Used in ProductImage component
<img src={imageUrl || window.ML4E?.placeholderImg} />
```
...
- Root plugin file (`mosaic-layouts-for-elementor.php`)
