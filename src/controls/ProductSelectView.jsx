 ...
 * @type {string}
 */
const PRODUCTS_ENDPOINT = '/mpl4e/v1/products';
...
const ProductSelectView = ({ initialValue, onChange }) => {
	...
	useEffect(() => {
		const fetchInitialProducts = async () => {
			try {
				...
				console.error('MPL4E: Failed to fetch initial products:', error);
				setDefaultOptions([]);
			} finally {
				setIsLoading(false);
			}
			...
			return new Promise((resolve) => {
				...
				debounceTimerRef.current = setTimeout(async () => {
					try {
						...
						console.error('MPL4E: Product search failed:', error);
						resolve([]);
					}
				}, DEBOUNCE_DELAY);
			});
	...
	const noOptionsMessage = useCallback(({ inputValue }) => {
		if (!inputValue || inputValue.length < MIN_SEARCH_LENGTH) {
			return __('Type to search…', 'mosaic-product-layouts-for-elementor');
		}
		return __('No products found', 'mosaic-product-layouts-for-elementor');
	}, []);
	...
	const loadingMessage = useCallback(
		() => __('Searching…', 'mosaic-product-layouts-for-elementor'),
		[]
	);
	...
	const formatOptionLabel = useCallback((option) => {
		return (
			<div className="mpl4e-product-option">
				<span className="mpl4e-product-option__label">{option.label}</span>
				<span className="mpl4e-product-option__id">#{option.value}</span>
			</div>
		);
	}, []);

	return (
		<div className="mpl4e-product-select-wrapper">
			<AsyncSelect
				classNamePrefix="mpl4e-ps"
