...
class Focal_Point extends Base_Data_Control {
	 ...
	 * The React component will mount into the .mpl4e-focal-point-container element.
	 *
	 * @since 1.0.0
	 * @access public
	 */
		...
		<# } #>
			...
			<div class="elementor-control-input-wrapper elementor-control-unit-5">
				<# var xValue=data.controlValue && typeof data.controlValue.x !=='undefined' ? data.controlValue.x : 50;
					...
					<input type="hidden" id="<?php echo esc_attr( $x_uid ); ?>" class="mpl4e-focal-point-x"
						data-setting="x" value="{{ xValue }}" />
					<input type="hidden" id="<?php echo esc_attr( $y_uid ); ?>" class="mpl4e-focal-point-y"
						data-setting="y" value="{{ yValue }}" />

					<!-- React mount container -->
					<div class="mpl4e-focal-point-container" data-control-uid="<?php echo esc_attr( $control_uid ); ?>"
						data-initial-x="{{ xValue }}" data-initial-y="{{ yValue }}"
						data-preview-image="{{ data.preview_image }}">
					</div>

					<!-- Value display -->
					<div class="mpl4e-focal-point-values">
						<span class="mpl4e-focal-point-value">
							X: <span class="mpl4e-x-value">{{ xValue }}</span>%
						</span>
						<span class="mpl4e-focal-point-value">
							Y: <span class="mpl4e-y-value">{{ yValue }}</span>%
						</span>
					</div>
			</div>
</div>
